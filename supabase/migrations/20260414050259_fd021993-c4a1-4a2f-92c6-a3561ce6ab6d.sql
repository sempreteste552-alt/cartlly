-- Update the handle_referral_completion function to match the actual order statuses
...
DECLARE
    ...
    v_customer RECORD;
BEGIN
    -- Only proceed if status is processando (confirmed), enviado (shipped), or entregue (delivered)
    IF NEW.status NOT IN ('processando', 'enviado', 'entregue') THEN
        RETURN NEW;
    END IF;
