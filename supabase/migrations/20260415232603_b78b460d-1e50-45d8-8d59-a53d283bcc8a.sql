-- Update the trigger function to use the correct schema for gen_random_bytes
...
BEGIN
    ...
    IF v_token IS NULL THEN
        -- Generate a random hex token (32 bytes = 64 characters)
        -- Using extensions schema where the function is located
        v_token := encode(extensions.gen_random_bytes(32), 'hex');
        INSERT INTO public.email_unsubscribe_tokens (email, token)
        VALUES (NEW.email, v_token);
    END IF;
