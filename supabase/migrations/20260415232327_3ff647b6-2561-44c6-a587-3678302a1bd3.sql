-- Create a function to handle sending invitation emails
...
BEGIN
    ...
    -- Note: We MUST include idempotency_key for transactional emails
    ...
    -- Enqueue the email using the existing wrapper
    -- We use SECURITY DEFINER to ensure we have permission to call enqueue_email
...
-- Create the trigger
DROP TRIGGER IF EXISTS trigger_on_store_invitation_created ON public.store_invitations;
CREATE TRIGGER trigger_on_store_invitation_created
AFTER INSERT ON public.store_invitations
FOR EACH ROW
EXECUTE FUNCTION public.on_store_invitation_created();
