...
import { AlertTriangle, Users, CreditCard, Gift, Search, Filter, Flag, ShieldOff, ShieldCheck, Ban, MousePointerClick, TrendingUp } from "lucide-react";
...
export default function SuperAdminIndicacoes() {
  ...
  const filtered = useMemo(() => {
    ...
    if (tenantSearch.trim()) {
      ...
      list = list.filter((r: any) => r.referrer_tenant_id?.toLowerCase().includes(q) || r.referral_code?.toLowerCase().includes(q));
    }
    return list;
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">Acompanhe, audite e gerencie todas as indicações do sistema.</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        ...
        <Card><CardContent className="pt-4 pb-3 px-4">
          <div className="flex items-center gap-2 text-muted-foreground text-xs mb-1"><TrendingUp className="h-3.5 w-3.5" /> Total Referrals</div>
            ...
            <CardContent className="p-4">
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <Select value={paymentFilter} onValueChange={setPaymentFilter}>
                  <SelectTrigger className="h-9 text-sm w-44"><SelectValue placeholder="Pagamento" /></SelectTrigger>
                  <SelectContent>
                ...
                <Select value={fraudFilter} onValueChange={setFraudFilter}>
                  <SelectTrigger className="h-9 text-sm w-44"><SelectValue placeholder="Fraude" /></SelectTrigger>
                  <SelectContent>
                ...
                <div className="relative flex-1 min-w-[180px]">
                  ...
                  <Input placeholder="Email do indicado..." value={emailSearch} onChange={(e) => setEmailSearch(e.target.value)} className="pl-9 h-9 text-sm" />
                </div>
                <div className="relative min-w-[180px]">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                  <Input placeholder="Tenant ID ou código..." value={tenantSearch} onChange={(e) => setTenantSearch(e.target.value)} className="pl-9 h-9 text-sm" />
                </div>
              </div>
            </CardContent>
          </Card>
                      ...
                      {filtered.map((r: any) => {
                        ...
                        return (
                          <TableRow key={r.id} className={r.flagged ? "bg-destructive/5" : ""}>
                            <TableCell className="text-xs font-mono text-muted-foreground truncate max-w-[120px]" title={r.referrer_tenant_id}>
                              {r.referrer_tenant_id?.substring(0, 8)}...
                            </TableCell>
                            <TableCell className="text-sm font-medium">{r.referred_email || "—"}</TableCell>
                            <TableCell className="text-xs font-mono">{r.referral_code}</TableCell>
                            ...
                            <TableCell className="text-xs text-muted-foreground truncate max-w-[100px]" title={r.ip_address || ""}>
                              {r.ip_address || "—"}
                            </TableCell>
                            <TableCell>
                    ...
                    <TableBody>
                      {discounts.map((d: any) => (
                        <TableRow key={d.id}>
                          ...
                          <TableCell className="text-xs text-muted-foreground">{format(new Date(d.created_at), "dd/MM/yy")}</TableCell>
                          <TableCell>
                            {d.amount > 0 && (
                              <Button
                                size="sm"
