    ...
    <div className={cn("flex flex-wrap items-center justify-center gap-2", className)}>
      {acceptedMethods.map((method) => {
        ...
        return (
          <div 
            key={method} 
            className="bg-white rounded px-2 py-1.5 flex items-center justify-center w-16 h-9 border border-gray-200 shadow-sm transition-transform hover:scale-110 shrink-0"
            title={data.label}
          >
            <img 
              src={data.icon} 
